...
--DELETE FROM CHAMPIONS_LEAGUE.BUTS WHERE saison_id= @SaisonID;;
--DELETE FROM CHAMPIONS_LEAGUE.MATCHS WHERE saison_id= @SaisonID;;
DELETE FROM CHAMPIONS_LEAGUE.JOUEURS_EQUIPE_SAISON WHERE saison_id= @SaisonID;
DELETE FROM CHAMPIONS_LEAGUE.ENTRAINEURS_EQUIPE_SAISON WHERE saison_id= @SaisonID;;
DELETE FROM CHAMPIONS_LEAGUE.JOURNEES;
DELETE FROM CHAMPIONS_LEAGUE.EQUIPES;
DELETE FROM CHAMPIONS_LEAGUE.JOUEURS;
DELETE FROM CHAMPIONS_LEAGUE.ENTRAINEURS;
DELETE FROM CHAMPIONS_LEAGUE.STADES;
DELETE FROM CHAMPIONS_LEAGUE.POSTES_REFERENCE;
DELETE FROM CHAMPIONS_LEAGUE.TYPES_BUTS_REFERENCE;
DELETE FROM CHAMPIONS_LEAGUE.SAISONS;

-- Réinitialisation des compteurs IDENTITY
DBCC CHECKIDENT ('CHAMPIONS_LEAGUE.JOUEURS', RESEED, 0);
DBCC CHECKIDENT ('CHAMPIONS_LEAGUE.ENTRAINEURS', RESEED, 0);
DBCC CHECKIDENT ('CHAMPIONS_LEAGUE.STADES', RESEED, 0);
DBCC CHECKIDENT ('CHAMPIONS_LEAGUE.SAISONS', RESEED, 0);
DBCC CHECKIDENT ('CHAMPIONS_LEAGUE.MATCHS', RESEED, 0);
GO
...
SET IDENTITY_INSERT CHAMPIONS_LEAGUE.STADES ON;
INSERT INTO CHAMPIONS_LEAGUE.STADES (stade_id, nomStade, villeStade, paysStade, capacite) VALUES
...
SET IDENTITY_INSERT CHAMPIONS_LEAGUE.STADES OFF;

/* TABLE POSTES_REFERENCE */
INSERT INTO CHAMPIONS_LEAGUE.POSTES_REFERENCE (poste_id, nomPoste) VALUES
(1, 'Gardien'), (2, 'Défenseur'), (3, 'Milieu'), (4, 'Attaquant');

/* TABLE TYPES_BUTS_REFERENCE */
INSERT INTO CHAMPIONS_LEAGUE.TYPES_BUTS_REFERENCE (type_but_id, nom_type_but) VALUES
...
SET IDENTITY_INSERT CHAMPIONS_LEAGUE.ENTRAINEURS ON;
INSERT INTO CHAMPIONS_LEAGUE.ENTRAINEURS (entraineur_id, nomEntraineur, prenomEntraineur, paysEntraineur) VALUES
...
SET IDENTITY_INSERT CHAMPIONS_LEAGUE.ENTRAINEURS OFF;

/* TABLE SAISONS  */
SET IDENTITY_INSERT CHAMPIONS_LEAGUE.SAISONS ON;
...
INSERT INTO CHAMPIONS_LEAGUE.SAISONS (saison_id, annee_debut, annee_fin)
SELECT SaisonID, AnneeDebut, AnneeDebut + 1 FROM AnneesSeries
OPTION (MAXRECURSION 0);
SET IDENTITY_INSERT CHAMPIONS_LEAGUE.SAISONS OFF;
GO
...
SET IDENTITY_INSERT CHAMPIONS_LEAGUE.JOUEURS ON;
INSERT INTO CHAMPIONS_LEAGUE.JOUEURS (joueur_id, nomJoueur, prenomJoueur, date_naissance, poste_id, paysJoueur) VALUES
...
SET IDENTITY_INSERT CHAMPIONS_LEAGUE.JOUEURS OFF;
GO

/* EQUIPES */
-- Adaptation IDENTITY_INSERT car vous avez spécifié equipe_id manuellement
SET IDENTITY_INSERT CHAMPIONS_LEAGUE.EQUIPES ON;
INSERT INTO CHAMPIONS_LEAGUE.EQUIPES (equipe_id, nomEquipe, abreviation, stade_id) VALUES
...
SET IDENTITY_INSERT CHAMPIONS_LEAGUE.EQUIPES OFF;
GO
...
DECLARE SaisonCursor CURSOR FOR SELECT saison_id, annee_debut FROM CHAMPIONS_LEAGUE.SAISONS;
...
INSERT INTO CHAMPIONS_LEAGUE.JOURNEES (saison_id, numero_journee, date_limite)
SELECT saison_id, numero_journee, date_calculee FROM #JourneesGenerales;
GO

/* TABLE SAISON_EQUIPES */

DECLARE @SaisonID INT = 1;
DECLARE @DateDebutSaison1 DATE = (SELECT DATEFROMPARTS(annee_debut, 7, 1) FROM CHAMPIONS_LEAGUE.SAISONS WHERE saison_id = 1);
DECLARE @NbEquipes INT = 16;

/* Nettoyage pour la saison 1 */
DELETE FROM CHAMPIONS_LEAGUE.SAISON_EQUIPES WHERE saison_id = @SaisonID;

/* Sélection des 16 élus pour la saison */
INSERT INTO CHAMPIONS_LEAGUE.SAISON_EQUIPES (saison_id, equipe_id)
SELECT TOP(@NbEquipes) @SaisonID, equipe_id 
FROM CHAMPIONS_LEAGUE.EQUIPES 
ORDER BY equipe_id ASC;

/* TABLE  JOUEURS_EQUIPE_SAISON*/
/*  Placement des 620 joueurs :20 joueurs par équipe */
WITH PlayerTeamMapping AS (
    SELECT joueur_id, 'PSG' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 1 AND 20 UNION ALL
    SELECT joueur_id, 'OM'  AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 21 AND 40 UNION ALL
    SELECT joueur_id, 'RCL' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 41 AND 60 UNION ALL
    SELECT joueur_id, 'RMA' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 61 AND 80 UNION ALL
    SELECT joueur_id, 'FCB' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 81 AND 100 UNION ALL
    SELECT joueur_id, 'WAC' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 101 AND 120 UNION ALL
    SELECT joueur_id, 'RCA' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 121 AND 140 UNION ALL
    SELECT joueur_id, 'MUN' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 141 AND 160 UNION ALL
    SELECT joueur_id, 'MCI' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 161 AND 180 UNION ALL
    SELECT joueur_id, 'LIV' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 181 AND 200 UNION ALL
    SELECT joueur_id, 'BAY' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 201 AND 220 UNION ALL
    SELECT joueur_id, 'BVB' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 221 AND 240 UNION ALL
    SELECT joueur_id, 'AJAX' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 241 AND 260 UNION ALL
    SELECT joueur_id, 'PSV' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 261 AND 280 UNION ALL
    SELECT joueur_id, 'ACM' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 281 AND 300 UNION ALL
    SELECT joueur_id, 'INT' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 301 AND 320 UNION ALL
    SELECT joueur_id, 'JUV' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 321 AND 340 UNION ALL
    SELECT joueur_id, 'NAP' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 341 AND 360 UNION ALL
    SELECT joueur_id, 'SLB' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 361 AND 380 UNION ALL
    SELECT joueur_id, 'FCP' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 381 AND 400 UNION ALL
    SELECT joueur_id, 'PAO' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 401 AND 420 UNION ALL
    SELECT joueur_id, 'GAL' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 421 AND 440 UNION ALL
    SELECT joueur_id, 'FEN' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 441 AND 460 UNION ALL
    SELECT joueur_id, 'SDK' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 461 AND 480 UNION ALL
    SELECT joueur_id, 'DYN' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 481 AND 500 UNION ALL
    SELECT joueur_id, 'AND' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 501 AND 520 UNION ALL
    SELECT joueur_id, 'BRU' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 521 AND 540 UNION ALL
    SELECT joueur_id, 'CEL' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 541 AND 560 UNION ALL
    SELECT joueur_id, 'RAN' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 561 AND 580 UNION ALL
    SELECT joueur_id, 'BAS' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 581 AND 600 UNION ALL
    SELECT joueur_id, 'RBS' AS abr FROM CHAMPIONS_LEAGUE.JOUEURS WHERE joueur_id BETWEEN 601 AND 620
)

INSERT INTO CHAMPIONS_LEAGUE.JOUEURS_EQUIPE_SAISON (saison_id, joueur_id, equipe_id, date_arrivee)
SELECT @SaisonID, ptm.joueur_id, eq.equipe_id, @DateDebutSaison1
FROM PlayerTeamMapping ptm 
JOIN CHAMPIONS_LEAGUE.EQUIPES eq ON ptm.abr = eq.abreviation;

/* TABLE  ENTRAINEURS_EQUIPE_SAISON */ 
/* Placement des 31 entraîneurs */
INSERT INTO CHAMPIONS_LEAGUE.ENTRAINEURS_EQUIPE_SAISON (saison_id, entraineur_id, equipe_id, date_arrivee)
SELECT 1, e.entraineur_id, eq.equipe_id, @DateDebutSaison1
FROM (SELECT entraineur_id, ROW_NUMBER() OVER (ORDER BY NEWID()) as rn FROM CHAMPIONS_LEAGUE.ENTRAINEURS) e
JOIN (SELECT equipe_id, ROW_NUMBER() OVER (ORDER BY NEWID()) as rn FROM CHAMPIONS_LEAGUE.EQUIPES) eq ON e.rn = eq.rn;


/* TABLE MATCHS  */
/* (Génération du calendrier et scores) */
DECLARE @NbRounds INT = 15; 
DECLARE @IntervalleJournee INT = 7; 
DECLARE @DateBaseJournee DATE = (SELECT DATEFROMPARTS(annee_debut, 7, 1) FROM CHAMPIONS_LEAGUE.SAISONS WHERE saison_id = @SaisonID);
...
FROM CHAMPIONS_LEAGUE.SAISON_EQUIPES SE
...
INSERT INTO CHAMPIONS_LEAGUE.MATCHS (saison_id, numero_journee, equipe_domicile_id, equipe_exterieur_id, stade_id, date_match, buts_domicile, buts_exterieur)
...
JOIN CHAMPIONS_LEAGUE.EQUIPES EQ_DOM ON FS.dom_id = EQ_DOM.equipe_id;


/* TABLE BUTS */

DELETE FROM CHAMPIONS_LEAGUE.BUTS WHERE match_id IN (SELECT match_id FROM CHAMPIONS_LEAGUE.MATCHS WHERE saison_id = 1);
...
SELECT M.match_id, M.equipe_domicile_id, M.equipe_exterieur_id, 
       CASE WHEN RAND(CHECKSUM(NEWID())) < 0.01 THEN 'CSC' 
            ...
            ELSE 'Pied' END
FROM CHAMPIONS_LEAGUE.MATCHS M CROSS JOIN Nums N 
...
INSERT INTO CHAMPIONS_LEAGUE.BUTS (match_id, joueur_id, equipe_but_id, minute_but, type_but_id)
SELECT 
    TB.match_id,
    COALESCE(
        (SELECT TOP 1 J.joueur_id FROM CHAMPIONS_LEAGUE.JOUEURS_EQUIPE_SAISON J 
         WHERE J.saison_id = 1 AND J.equipe_id = (CASE WHEN TB.type_nom = 'CSC' THEN TB.equipe_adverse_id ELSE TB.equipe_but_id END)
         ORDER BY NEWID()), 
        (SELECT TOP 1 joueur_id FROM CHAMPIONS_LEAGUE.JOUEURS_EQUIPE_SAISON WHERE saison_id = 1)
    ),
    TB.equipe_but_id,
    CAST(RAND(CHECKSUM(NEWID())) * 90 AS INT) + 1,
    TBR.type_but_id
FROM #TempButs TB
JOIN CHAMPIONS_LEAGUE.TYPES_BUTS_REFERENCE TBR ON TB.type_nom = TBR.nom_type_but;
DROP TABLE #TempButs;